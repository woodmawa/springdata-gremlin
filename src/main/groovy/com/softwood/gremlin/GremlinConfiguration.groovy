...
class GremlinConfiguration extends AbstractGremlinConfiguration {
    ...
    GremlinConfig getGremlinConfig() {
        ...
        def config = new GremlinConfig.GremlinConfigBuilder().endpoint (remoteConnectionEndpoint)
        .port (port).username(uname).password(pwd).sslEnabled(false).telemetryAllowed(true)
        //.serializer("GRYO_V1D0").build()
        .build()
